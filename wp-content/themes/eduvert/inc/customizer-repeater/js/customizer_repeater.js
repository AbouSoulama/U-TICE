/* global jQuery */
/* global wp */
function eduvert_media_upload(button_class) {
    'use strict';
 ...
 *** Generate unique id ***
 *********************************************/
function eduvert_customizer_repeater_uniqid(prefix, more_entropy) {
    'use strict';
 ...
 *** General Repeater ***
 *********************************************/
function eduvert_customizer_repeater_refresh_social_icons(th) {
    'use strict';
    ...
    th.find('.customizer-repeater-social-repeater-container').each(function () {
        ...
        if (!id) {
            id = 'customizer-repeater-social-repeater-' + eduvert_customizer_repeater_uniqid();
    ...
    eduvert_customizer_repeater_refresh_general_control_values();
}


function eduvert_customizer_repeater_refresh_general_control_values() {
    'use strict';
    jQuery('.customizer-repeater-general-control-repeater').each(function () {
        ...
        th.find('.customizer-repeater-general-control-repeater-container').each(function () {
            ...
            if (!id) {
                id = 'social-repeater-' + eduvert_customizer_repeater_uniqid();
            ...
            if (text !== '' || image_url !== '' || image_url2 !== '' || title !== '' || subtitle !== '' || icon_value !== '' || link !== '' || choice !== '' || social_repeater !== '' || shortcode !== '' || slide_align !== '' || color !== '') {
                values.push({
                    ...
                    'color': color,
                    'color2': color2,
                    'text': eduvertescapeHtml(text),
                    'link': link,
                    'text2': eduvertescapeHtml(text2),
					'button_second': eduvertescapeHtml(button_second),
                    'link2': link2,
                    'image_url': (choice === 'customizer_repeater_none' ? '' : image_url),
                    'choice': choice,
                    'image_url2': image_url2,
                    'title': eduvertescapeHtml(title),
                    'subtitle': eduvertescapeHtml(subtitle),
					'subtitle2': eduvertescapeHtml(subtitle2),
					'slide_align': eduvertescapeHtml(slide_align),
					'open_new_tab' : open_new_tab,
                    'social_repeater': eduvertescapeHtml(social_repeater),
                    'id': id,
                    'shortcode': eduvertescapeHtml(shortcode)
                });
            }
...
jQuery(document).ready(function () {
    'use strict';
    var eduvert_theme_controls = jQuery('#customize-theme-controls');
    eduvert_theme_controls.on('click', '.customizer-repeater-customize-control-title', function () {
    ...
    eduvert_theme_controls.on('change', '.icp',function(){
        eduvert_customizer_repeater_refresh_general_control_values();
        return false;
    });
	
	eduvert_theme_controls.on('change','.customizer-repeater-slide-align', function(){
		eduvert_customizer_repeater_refresh_general_control_values();
        return false;
		
	});

    eduvert_theme_controls.on('change','.customizer-repeater-image2-control', function(){
        eduvert_customizer_repeater_refresh_general_control_values();
        return false;
        
    });
	
    eduvert_theme_controls.on('change', '.customizer-repeater-image-choice', function () {
    ...
    eduvert_media_upload('.customizer-repeater-custom-media-button');
     ...
     *
     */
    eduvert_theme_controls.on('click', '.customizer-repeater-new-field', function () {
        var split_add_more_button=jQuery(this).text();
        var split_add_more_button_split=split_add_more_button.substr(4, 12);
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-' + eduvert_customizer_repeater_uniqid();
		
		if(split_add_more_button_split=="Add New Soci")
        {
        if(jQuery('#exist_eduvert_Social').val()>=3)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_eduvert_Social').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eduvert_Social').val())+1;
         jQuery('#exist_eduvert_Social').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Slid")
        {
        if(jQuery('#exist_eduvert_Slider').val()>=3)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_eduvert_Slider').val()<3)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eduvert_Slider').val())+1;
         jQuery('#exist_eduvert_Slider').val(new_service_add_val);  
        }
        }
		
		 if(split_add_more_button_split=="Add New Cour")
        {
        if(jQuery('#exist_eduvert_Course').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_eduvert_Course').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eduvert_Course').val())+1;
         jQuery('#exist_eduvert_Course').val(new_service_add_val);  
        }
        }
         if(split_add_more_button_split=="Add New Funf")
        {
        if(jQuery('#exist_eduvert_Funfact').val()>=4)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_eduvert_Funfact').val()<4)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eduvert_Funfact').val())+1;
         jQuery('#exist_eduvert_Funfact').val(new_service_add_val);  
        }
        }
    ...
    eduvert_theme_controls.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_eduvert_Social').val(parseInt(jQuery('#exist_eduvert_Social').val())-1); 
                }
				
				if(split_delete_button_split=="Delete Slide")
                {
                jQuery('#exist_eduvert_Slider').val(parseInt(jQuery('#exist_eduvert_Slider').val())-1);  
                }
				if(split_delete_button_split=="Delete Cours")
                {
                jQuery('#exist_eduvert_Course').val(parseInt(jQuery('#exist_eduvert_Course').val())-1);  
                }
				if(split_delete_button_split=="Delete Funfa")
                {
                jQuery('#exist_eduvert_Funfact').val(parseInt(jQuery('#exist_eduvert_Funfact').val())-1); 
                }
            });
        }
        return false;
    ...
    eduvert_theme_controls.on('keyup', '.customizer-repeater-title-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });
    ...
    eduvert_theme_controls.on('keyup', '.customizer-repeater-subtitle-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });
	
	 eduvert_theme_controls.on('keyup', '.customizer-repeater-subtitle2-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });
	
    eduvert_theme_controls.on('keyup', '.customizer-repeater-shortcode-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });

    eduvert_theme_controls.on('keyup', '.customizer-repeater-text-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });

    eduvert_theme_controls.on('keyup', '.customizer-repeater-link-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });

    eduvert_theme_controls.on('keyup', '.customizer-repeater-text2-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });
	
	eduvert_theme_controls.on('keyup', '.customizer-repeater-button2-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });

    eduvert_theme_controls.on('keyup', '.customizer-repeater-link2-control', function () {
        eduvert_customizer_repeater_refresh_general_control_values();
    });
	
	eduvert_theme_controls.on('change','.customizer-repeater-checkbox', function(){
    ...
    /*----------------- Socials Repeater ---------------------*/
    eduvert_theme_controls.on('click', '.social-repeater-add-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var id = 'customizer-repeater-social-repeater-' + eduvert_customizer_repeater_uniqid();
        if (typeof th !== 'undefined') {
    ...
    eduvert_theme_controls.on('click', '.social-repeater-remove-social-item', function (event) {
        event.preventDefault();
        var th = jQuery(this).parent();
        var repeater = jQuery(this).parent().parent();
        th.remove();
        eduvert_customizer_repeater_refresh_social_icons(repeater);
        return false;
    });

    eduvert_theme_controls.on('keyup', '.customizer-repeater-social-repeater-link', function (event) {
        event.preventDefault();
        var repeater = jQuery(this).parent().parent();
        eduvert_customizer_repeater_refresh_social_icons(repeater);
        return false;
    ...
    eduvert_theme_controls.on('change', '.customizer-repeater-social-repeater-container .icp', function (event) {
        event.preventDefault();
...
function eduvertescapeHtml(string) {
    'use strict';
    //noinspection JSUnresolvedFunction
    string = String(string).replace(new RegExp('\r?\n', 'g'), '<br />');
    string = String(string).replace(/\\/g, '&#92;');
    return String(string).replace(/[&<>"'\/]/g, function (s) {
        return eduvertentityMap[s];
    });
